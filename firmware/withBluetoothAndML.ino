...
const float f_low = 3.0f;
const float f_high = 12.0f;


// Per-axis Butterworth filters
...
const float freq_peak_ratio = 5.0f;
const float min_peak_mag = 1e-3f;
...
void loop() {
  ...
  const unsigned long targetInterval = 1000000UL / fs;
  ...
  if (rmsFilled) {
    ...
    Serial.print("RMS X: "); Serial.print(rmsX, 2);
    Serial.print("°, Y: "); Serial.print(rmsY, 2);
    Serial.print("°, Z: "); Serial.print(rmsZ, 2);
    Serial.println("°/s");


    SerialBT.print("RMS X: "); SerialBT.print(rmsX, 2);
    SerialBT.print("°, Y: "); SerialBT.print(rmsY, 2);
    SerialBT.print("°, Z: "); SerialBT.print(rmsZ, 2);
    SerialBT.println("°/s");
  }
  ...
  SerialBT.print("Dominant Freq X: ");
  if (freqX > 0.0f) {
    Serial.print(freqX, 2); SerialBT.print(freqX, 2);
  } else {
    Serial.print("n/a"); SerialBT.print("n/a");
  }
  ...
  SerialBT.print(" Hz, Y: ");
  if (freqY > 0.0f) {
    Serial.print(freqY, 2); SerialBT.print(freqY, 2);
  } else {
    Serial.print("n/a"); SerialBT.print("n/a");
  }
  ...
  SerialBT.print(" Hz, Z: ");
  if (freqZ > 0.0f) {
    Serial.print(freqZ, 2); SerialBT.print(freqZ, 2);
  } else {
    Serial.print("n/a"); SerialBT.print("n/a");
  }
  ...
  if (rms_y <= 37.33f) {
    classification = 0.0f; // normal
  } else {
    if (freqY <= 1.56f) {
      classification = 0.0f; // normal
    } else {
      classification = 1.0f; // tremor
    }
  }


  // Display classification
  Serial.print("Tremor Classification: ");
  SerialBT.print("Tremor Classification: ");
  if (classification == 1.0f) {
    Serial.println("TREMOR");
    SerialBT.println("TREMOR");
  } else {
